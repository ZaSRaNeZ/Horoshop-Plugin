var arrru = new Array('Я', 'я', 'Ю', 'ю', 'Ч', 'ч', 'Ш', 'ш', 'Щ', 'щ', 'Ж', 'ж', 'А', 'а', 'Б', 'б', 'В', 'в', 'Г', 'г', 'Д', 'д', 'Е', 'е', 'Ё', 'ё', 'З', 'з', 'И', 'и', 'Й', 'й', 'К', 'к', 'Л', 'л', 'М', 'м', 'Н', 'н', 'О', 'о', 'П', 'п', 'Р', 'р', 'С', 'с', 'Т', 'т', 'У', 'у', 'Ф', 'ф', 'Х', 'х', 'Ц', 'ц', 'Ы', 'ы', 'Ь', 'ь', 'Ъ', 'ъ', 'Э', 'э', ' ','і','ї','ь');

var arren = new Array('Ya', 'ya', 'Yu', 'yu', 'Ch', 'ch', 'Sh', 'sh', 'Sh', 'sh', 'Zh', 'zh', 'A', 'a', 'B', 'b', 'V', 'v', 'G', 'g', 'D', 'd', 'E', 'e', 'E', 'e', 'Z', 'z', 'I', 'i', 'J', 'j', 'K', 'k', 'L', 'l', 'M', 'm', 'N', 'n', 'O', 'o', 'P', 'p', 'R', 'r', 'S', 's', 'T', 't', 'U', 'u', 'F', 'f', 'H', 'h', 'C', 'c', 'Y', 'y', '', '', '', '', 'E', 'e', '_','i','i','');
...
function start() {
	...
	*/
	...
	for (let ind = arrStartInd; ind < x.length; ind++) {
		...
		console.log(x[ind].indexOf("@@"));
		if (x[ind].indexOf("@@") >= 0 ){
			tableName = x[ind].replace(/@@/g,"");
			continue;

		} 


		sqlText += "SET @handlerTableName = (SELECT `table` FROM handlers WHERE `title` = '"+ tableName +"');"+
		
		"SET @colcheck =  (SELECT COUNT(COLUMN_NAME) AS EXIST FROM INFORMATION_SCHEMA.COLUMNS WHERE  TABLE_NAME = @handlerTableName AND `COLUMN_NAME` = '" + xEn + "');\n" +


			"SET @stmt = case @colcheck \n" +
			"WHEN 0 THEN \" ALTER TABLE @handlerTableName ADD `" + xEn + "` " + columnType + " NOT NULL; \"" +
			"ELSE \"SET @t=@t\"" +
			"END;\n" +
			...
			"SET @stmt = case @colcheck\n" +
			"WHEN 0 THEN \" INSERT INTO handlers_format (`parent`, `type`, `name`, `title`, `sortorder`, `group`, `localize`) VALUES ((SELECT id FROM handlers WHERE `table` = @handlerTableName LIMIT 1),'" + characType + "','" + xEn + "','" + x[ind] + "','1',(SELECT id FROM handlers_format_group WHERE `type` = (SELECT id FROM book_group_type WHERE `title` = 'Основная') AND `parent` = (SELECT id FROM handlers WHERE `table` = @handlerTableName) LIMIT 1),	'" + localizeParam + "'	);\"\n" +
			"ELSE \"SET @t=@t\"\n" +
			"END;\n" +
		...
		if (characType == 'select') {
			...
			sqlText += "SET @stmt = case @colcheck\n" +
				...
				"SELECT CONCAT ((SELECT `title` FROM handlers WHERE `table` = @handlerTableName), '-" + x[ind] + "') INTO @nameBook;\n" +
				...
				"SET @stmt = case @colcheck\n" +
				"WHEN 0 THEN \"INSERT INTO format_references (`parent`, `table`)  VALUES ((SELECT id FROM handlers_format WHERE name = '" + xEn + "' AND `parent` = (SELECT `id` FROM handlers WHERE `table` = @handlerTableName) LIMIT 1) , (SELECT `table` FROM books WHERE id = (SELECT max(id) FROM books LIMIT 1) LIMIT 1));\"\n" +
				"ELSE \"SET @t=@t\"\n" +
				"END;\n" +
